...
int main()
{
    ...
    while (i < 3)
    {
        (void)!std::scanf("Blueprint %d: Each ore robot costs %d ore. Each clay robot costs %d ore. Each obsidian robot costs %d ore and %d clay. Each geode robot costs %d ore and %d obsidian.\n",
                          &i, &ore_cost, &clay_cost, &obs_cost_ore, &obs_cost_clay, &geo_cost_ore, &geo_cost_obs);

        ore_max = std::max(std::max(ore_cost, clay_cost), std::max(obs_cost_ore, geo_cost_ore));
        cache.clear();
        total *= dfs(32, 1, 0, 0, 0, 0, 0);
    }
